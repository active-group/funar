      ...
      case GameCommand.PlayCard(player, card) =>
        if (playValid(tableState, player, card)) {
          ...
          if (turnOver(state1)) {
            ...
            val trickTaker = whoTakesTrick(trick1)
            val event2 = GameEvent.TrickTaken(trickTaker, trick1)
            val state2 = tableProcessEvent(event2, state1)
            val event3 = gameOver(state2) match {
              case Some(winner) => GameEvent.GameEnded(winner)
              case None => GameEvent.PlayerTurnChanged(trickTaker)
            }
