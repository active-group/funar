...
inBounds :: StackSet window -> Bool
inBounds stackSet = Map.member (current stackSet) (stacks stackSet)

invariant :: T -> Bool
invariant (stackSet :: T) =
    inBounds stackSet && noDuplicates (concat (Map.elems (stacks stackSet)))
