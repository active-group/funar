...
data Strategy monadT =
  Strategy (MonadTrans monadT => forall monad . Monad monad =>
               Player -> EventProcessor (monadT monad) GameEvent GameCommand)
...
chooseAlong _ playerState =
  case playerStateTrick playerState of
    [] -> return (Set.findMin (playerStateHand playerState))       -- kleine erste Karte
    trick ->
