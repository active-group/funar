module HeartsViewModel
  exposing (ViewModel, GameCommand(..), GameEvent(..), Player, PlayerName,
            Card, Hand, Pile, Trick, Rank(..), Suit(..), PlayerHands,
            emptyHand, emptyViewModel, deck, cardColor, prettyCard,
            tableProcessEvent,
            allSuits, allRanks,
            pileScore,
            recordCommands, getCommands)
...
type alias Pile = List Card

type alias PlayerPiles = Dict PlayerName Pile
...
emptyViewModel : List Player -> ViewModel
emptyViewModel players =
  { players = players
...
tableProcessEvent : GameEvent -> ViewModel -> ViewModel
tableProcessEvent event viewModel =
  case event of
    HandDealt player hand ->
      { viewModel | hands = Dict.insert player.name hand viewModel.hands,
                trick = emptyTrick }
-- exercise: leave out cases
    PlayerTurnChanged player ->
      { viewModel | players = rotateTo player viewModel.players }
    LegalCardPlayed player card ->
      { viewModel | players = rotate (rotateTo player viewModel.players),
                hands = takeCard viewModel.hands player card,
                piles = viewModel.piles,
                trick = addToTrick player card viewModel.trick }
    TrickTaken player trick ->
      { viewModel | piles = addToPile viewModel.piles player (cardsOfTrick trick),
                trick = emptyTrick }
    IllegalCardAttempted _ _ -> viewModel
    GameEnded player ->
      { viewModel | winner = Just player }
...
cardScore : Card -> Int
cardScore card =
  case (card.suit, card.rank) of
   (Spades, Queen) -> 13
   (Hearts, _) -> 1
   _ -> 0

pileScore : Pile -> Int
pileScore pile = List.sum (List.map cardScore pile)

recordCommands : List GameCommand -> ViewModel -> ViewModel
recordCommands commands viewModel =
  { viewModel | gameCommands = viewModel.gameCommands ++ commands }

getCommands : ViewModel -> (List GameCommand, ViewModel)
getCommands viewModel =
  (viewModel.gameCommands, { viewModel | gameCommands = [] })