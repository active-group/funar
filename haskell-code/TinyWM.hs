...
empty n = 
  StackSet { current = 0, 
             stacks  = Map.fromList (zip [0..n-1] (repeat [])) }
...
rotate direction stackSet = 
  stackSet { stacks = Map.adjust (rot direction) (current stackSet) (stacks stackSet) }

rot :: Ordering -> [a] -> [a]
rot direction [] = []
rot direction xs = case direction of
  GT -> tail xs ++ [head xs]
  LT -> last xs : init xs
  _ -> xs
...
insert window stackIndex stackSet = 
  let new = delete window stackSet
  in new { stacks = Map.adjust (window:) stackIndex (stacks new) }
