...
chooserStrategy' ::
  Chooser player effects -> Strategy '[Bundle (State (PlayerState player) ': effects)]
chooserStrategy' choose =
  \ player event ->
    do sendBundle (playerProcessGameEventM player event)
       playerState <- sendBundle State.get
       case event of
         HandDealt player' hand ->
           if (player == player') && (containsCard twoOfClubs hand)
           then return [PlayCard player twoOfClubs]
         ...
         PlayerTurnChanged player' ->
           if player == player'
           then do -- card <- sendBundle (raise (choose player playerState))
                   let card = undefined
                   return [PlayCard player card]
           else return []
         LegalCardPlayed player' card -> return []
...
type Chooser' player effect =
  Player -> PlayerState player -> Sem '[effect] Card


-- WIP
chooserStrategy'' :: forall player effect p b .
   Chooser' player effect -> Player -> p -> Sem '[Bundle '[effect, State (PlayerState player)]] b
chooserStrategy'' choose =
  \ player event ->
    -- (State (PlayerState player) ': effects
    -- Sem r a -> Sem '[Bundle r] a
    -- Sem effects a -- raise --> Sem (Bundle (... : effects) ': effects) a
    -- -- raise --> Sem (State PlayerState, Bundle (... : effects) ': effects) a
    do let playerState = undefined
       -- works:
       -- card <- sendBundle (raiseUnder @(Bundle '[effect, State (PlayerState player)]) (choose player playerState))
       card <- rewrite (injBundle @effect @'[effect, State (PlayerState player)])  (choose player playerState)
       return undefined
