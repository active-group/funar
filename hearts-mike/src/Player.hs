...
playerProcessEvent player (TrickTaken player' trick) state
  ...
  | otherwise =
    state { playerStateTrick = emptyTrick }
playerProcessEvent player (IllegalCardPlayed player' card) state = state
...
alongStrategy :: Member (State (PlayerState player)) effects 
                    => Strategy effects
...
chooseInteractive :: Member Teletype effects => Chooser player effects
