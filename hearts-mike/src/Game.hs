...
gameSem players =
  do
    ...
    let processCommand :: GameStepEffects effects => GameCommand -> Sem effects [GameEvent]
        processCommand command =
          do
            (next, state) <- State.get
            let (step, state') = runGameStep (next command) state
            let loop :: GameStepEffects effects => GameStep (Maybe Player) -> TableState -> [GameEvent] -> Sem effects [GameEvent]
                loop step state events =
                  case step of
