...
playCommand players gameCommand =
  do events <- gameCommandEventsM gameCommand
     ...
     then return ()
       ...
       do gameCommandss <- lift (mapM (\ gameEvent -> playEvent players gameEvent) events)
...
gameAlong =
  do let player1 = Player "1" "Mike"
     ...
     let rwt = runEventSourcingT (playGame players4 shuffledDeck) (emptyGameState playerNames)
     let ((((((), events, _), state4), state3), state2), state1) = Identity.runIdentity (State.runStateT (State.runStateT (State.runStateT (State.runStateT rwt emptyPlayerState) emptyPlayerState) emptyPlayerState) emptyPlayerState)
     return events
...
gameInteractive =
  do let player1 = Player "1" "Mike"
     ...
     shuffledDeck <- shuffle deck
     let rwt = runEventSourcingT (playGame players4 shuffledDeck) (emptyGameState playerNames)
     runRWTIO (runRWTIO (runRWTIO (runRWTIO (return . Identity.runIdentity) state1) state2) state3) state4 rwt
     return ()
