...
emptyPlayerState = PlayerState emptyHand emptyTrick []
...
statePlayerIO player playerM =
  do
    let (commands0, step0) = runStatePlayer player playerM emptyPlayerState []
...
chooseInteractive :: (StatePlayerMonad m, TtyPlayerMonad m) => Player -> m Card
chooseInteractive player =
  do
    playerState <- getPlayerStateM
    writeLineM ("Your turn, player " ++ (playerName player))
...
interactiveStrategy player = chooserStrategy player (chooseInteractive player)
...
stateTtyPlayerIO player playerM =
  do
    let (commands0, lines0, step0) = runStateTtyPlayer player playerM emptyPlayerState [] []
