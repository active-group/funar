...
playCommandC concurrentPlayers gameCommand =
  do events <- gameCommandEventsM gameCommand
     ...
     then return ()
       ...
       do commandss <- lift (mapM (\ event -> playEventC concurrentPlayers event) events)
...
gameInteractiveC =
  do let player1 = Player "1" "Mike"
     ...
     concurrentPlayers <- sequence (zipWith (\ player state ->
                                            ...
                                            players states)
     (result, events, state) <- runEventSourcingT (playGameC concurrentPlayers deck) (emptyGameState players)
     return events
